...
<template>
  ...
  <div class="centerNews"><h1>Announcement</h1><hr style="width:35%; min-width:600px;"><NewsCompo v-for="ann in this.annArrayData.slice(((currentPage-1)*pageSize),(currentPage*pageSize))" v-bind="ann" :key="ann.ann_id"> Announcement ID {{ann.ann_id}} : {{ann.ann_des}}</NewsCompo>
  <div class="center"><va-pagination v-model="currentPage" :pages="Math.ceil(annArrayData.length/pageSize)" :visible-pages="5"  /></div></div>
</template>
<style scope>
.centerNews{
  width: 100%;
